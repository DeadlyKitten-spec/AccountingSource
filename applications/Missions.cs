    ...
    public partial class Missions : Form
    {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (fdriver)
            {
                ...
                Line = "(SELECT * FROM `request` WHERE `dateAccept` BETWEEN '" + answer1 + "' AND '" + answer2 + "' AND `drivers` = '" + comboBox1.Text + "' AND `status` = 'Исполнена' AND `mission` = 'Да' ORDER BY `id` ASC) ORDER BY `object` ASC;";
                DB db = new DB();
            ...
            {
                Line = "(SELECT * FROM `request` WHERE `dateAccept` BETWEEN '" + answer1 + "' AND '" + answer2 + "' AND `driveCont` = '" + comboBox2.Text + "' AND `status` = 'Исполнена' AND `mission` = 'Да' ORDER BY `id` ASC) ORDER BY `object` ASC;";
                DB db = new DB();
                MySqlCommand command = new MySqlCommand(Line, db.getConnection());
                bool g = true;
                db.openConnection();
